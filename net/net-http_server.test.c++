...
auto register_server_tests()
{
    ...
    // Regression: waiting for handlers without shutdown() left idle keep-alive
    // connections (e.g. yarproxy→yardb pools) blocking forever on SIGTERM.
    tester::bdd::scenario("stop unblocks idle keep-alive handlers, [net]") = [] {
        if(not network_tests_enabled()) return;
        ...
        auto idle = std::optional<net::endpointstream>{};
        try
        {
            idle = net::connect("127.0.0.1", std::to_string(port));
            std::this_thread::sleep_for(50ms);
        }
        catch(...)
        {
    ...
    // CORS preflight must use options() + cors_middleware. Dispatching OPTIONS to
    // GET/POST handlers (removed) executed application logic on unwrapped routes.
    tester::bdd::scenario("OPTIONS CORS preflight reaches cors_middleware via options(), [net]") = [] {
        if(not network_tests_enabled()) return;

        tester::bdd::given("An HTTP server with POST and OPTIONS wrapped in cors_middleware") = [] {
            auto server = std::make_shared<http::server>();
            ...
            auto wrapped = cors_mw(post_handler);
            server->post("/api/data").response_with_headers("application/json", wrapped);
            server->options("/api/data").response_with_headers("application/json", wrapped);
            server->timeout(std::chrono::seconds{1});

            tester::bdd::when("Browser-style OPTIONS preflight is sent to the route") = [server, handler_invoked] {
                auto response_status = std::make_shared<std::string>("");
    ...
    // Regression: OPTIONS + Access-Control-Request-Method must not invoke a
    // registered POST/GET handler when no options() route exists.
    tester::bdd::scenario("OPTIONS preflight does not execute POST handler without options(), [net]") = [] {
        if(not network_tests_enabled()) return;

        tester::bdd::given("An HTTP server with a bare POST route (no CORS, no options)") = [] {
            auto server = std::make_shared<http::server>();
            auto handler_invoked = std::make_shared<std::atomic<bool>>(false);
            ...
            tester::bdd::when("OPTIONS preflight is sent with Access-Control-Request-Method") =
                ...
                tester::bdd::then("Response is 405 and the POST handler is not run") =
    ...
    // Regression: other hop-by-hop / framing-adjacent names must not be echoed
    // into the response either (Upgrade, Keep-Alive, TE, Trailer, proxy-*).
    tester::bdd::scenario("Custom response headers omit hop-by-hop names, [net]") = [] {
        if(not network_tests_enabled()) return;

        tester::bdd::given("A handler that returns hop-by-hop custom headers") = [] {
            auto server = std::make_shared<http::server>();
            server->get("/hop").response_handler("text/plain",
                [](std::string_view, std::string_view, http::headers&) {
                    auto out = http::headers{};
                    out.set("Upgrade"s, "websocket"s);
                    out.set("Keep-Alive"s, "timeout=5"s);
                    out.set("TE"s, "trailers"s);
                    out.set("Trailer"s, "Expires"s);
                    out.set("Proxy-Connection"s, "keep-alive"s);
                    out.set("Proxy-Authenticate"s, "Basic"s);
                    out.set("Proxy-Authorization"s, "Basic abc"s);
                    out.set("Transfer-Encoding"s, "chunked"s);
                    out.set("x-ok"s, "1"s);
                    return http::make_response(http::status_ok, "OK"s, std::make_optional(out));
                });
            server->timeout(std::chrono::seconds{1});
            ...
            tester::bdd::when("A GET hits the route") = [server] {
                ...
                tester::bdd::then("Hop-by-hop names are omitted and safe custom header remains") =
                    [response_headers] {
                        check_contains(*response_headers, "x-ok: 1");
                        check_contains(*response_headers, "Content-Length: 2");
                        check_false(response_headers->contains("Upgrade"));
                        check_false(response_headers->contains("Keep-Alive"));
                        check_false(response_headers->contains("TE:"));
                        check_false(response_headers->contains("Trailer"));
                        check_false(response_headers->contains("Proxy-Connection"));
                        check_false(response_headers->contains("Proxy-Authenticate"));
                        check_false(response_headers->contains("Proxy-Authorization"));
                        check_false(response_headers->contains("Transfer-Encoding"));
                        check_false(response_headers->contains("chunked"));
                    };
            };
        };
    };
