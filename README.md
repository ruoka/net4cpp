...
`host` / `port` / `path` must not contain CR, LF, or NUL (rejected before connect —
they would inject headers into the upgrade request). `close()` waits briefly for the
peer close frame (including truncated frames), then force-closes (default 2s;
